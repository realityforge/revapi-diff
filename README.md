...
Alternatively it can be directly downloaded from [http://central.maven.org/maven2/org/realityforge/revapi/diff/revapi-diff/0.03/revapi-diff-0.03-all.jar](http://central.maven.org/maven2/org/realityforge/revapi/diff/revapi-diff/0.03/revapi-diff-0.03-all.jar)
as the package has been built with all dependencies included. It is an executable jar file and can be run
by passing it as an argument to the `java -jar revapi-diff-0.03-all.jar ...`.  To get a list of options that
can be passed to the program run `java -jar revapi-diff-0.03-all.jar --help`.
...
```bash
java -jar revapi-diff-0.03-all.jar \
