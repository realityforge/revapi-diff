...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/revapi-diff/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/revapi-diff/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      unless candidates.empty?
        ...
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/revapi-diff', milestone[:number], :state => 'closed')
        end
      end
    end
  end
